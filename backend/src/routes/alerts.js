...
router.post(
  '/bulk-import',
  wrapAsync(async (req, res) => {
    await AlertsService.bulkImport(req, res, true, req.headers.referer);
    const payload = true;
    res.status(200).send(payload);
  }),
);
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    const payload = await AlertsDBApi.findAll(req.query, globalAccess);
    if (filetype && filetype === 'csv') {
      const fields = ['id', 'description', 'alert_time'];
      const opts = { fields };
      try {
...
router.get(
  '/:id',
  wrapAsync(async (req, res) => {
    const payload = await AlertsDBApi.findBy({ id: req.params.id });
