  ...
  static async search(searchQuery, currentUser, organizationId, globalAccess) {
    try {
      ...
      const tableColumns = {
        ...
        corporate_sponsors: ['company_name', 'industry', 'csr_focus'],

        educational_contents: ['title', 'content'],

        regions: ['name', 'coordinates'],

        restoration_projects: ['project_name', 'description'],

        organizations: ['name'],
      };
      const columnsInt = {
        restoration_projects: ['funding_goal', 'current_funding'],
      };
