...
import { loadColumns } from './configureEducational_contentsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';

const perPage = 10;

const TableSampleEducational_contents = ({
  filterItems,
  setFilterItems,
  filters,
}) => {
  ...
  const {
    ...
    loading,
    count,
    notify: educational_contentsNotify,
    refetch,
  } = useAppSelector((state) => state.educational_contents);
  ...
  useEffect(() => {
    if (educational_contentsNotify.showNotification) {
      notify(
        educational_contentsNotify.typeNotification,
        educational_contentsNotify.textNotification,
      );
    }
  }, [educational_contentsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/educational_contents/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/educational_contents/educational_contents-view/?id=${id}`);
  };
...
export default TableSampleEducational_contents;
