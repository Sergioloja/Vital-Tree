...
import { loadColumns } from './configureRestoration_projectsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';

const perPage = 10;

const TableSampleRestoration_projects = ({
  filterItems,
  setFilterItems,
  filters,
}) => {
  ...
  const {
    ...
    loading,
    count,
    notify: restoration_projectsNotify,
    refetch,
  } = useAppSelector((state) => state.restoration_projects);
  ...
  useEffect(() => {
    if (restoration_projectsNotify.showNotification) {
      notify(
        restoration_projectsNotify.typeNotification,
        restoration_projectsNotify.textNotification,
      );
    }
  }, [restoration_projectsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/restoration_projects/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/restoration_projects/restoration_projects-view/?id=${id}`);
  };
...
export default TableSampleRestoration_projects;
