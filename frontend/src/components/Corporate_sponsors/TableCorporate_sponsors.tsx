...
import { loadColumns } from './configureCorporate_sponsorsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';

const perPage = 10;

const TableSampleCorporate_sponsors = ({
  filterItems,
  setFilterItems,
  filters,
}) => {
  ...
  const {
    ...
    loading,
    count,
    notify: corporate_sponsorsNotify,
    refetch,
  } = useAppSelector((state) => state.corporate_sponsors);
  ...
  useEffect(() => {
    if (corporate_sponsorsNotify.showNotification) {
      notify(
        corporate_sponsorsNotify.typeNotification,
        corporate_sponsorsNotify.textNotification,
      );
    }
  }, [corporate_sponsorsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/corporate_sponsors/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/corporate_sponsors/corporate_sponsors-view/?id=${id}`);
  };
...
export default TableSampleCorporate_sponsors;
