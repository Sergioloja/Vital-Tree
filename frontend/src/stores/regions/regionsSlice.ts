...
export const fetch = createAsyncThunk('regions/fetch', async (data: any) => {
  const { id, query } = data;
  const result = await axios.get(`regions${query || (id ? `/${id}` : '')}`);
  return id
    ? result.data
...
export const uploadCsv = createAsyncThunk(
  ...
  async (file: File, { rejectWithValue }) => {
    try {
      ...
      const result = await axios.post('regions/bulk-import', data, {
        headers: {
...
export const update = createAsyncThunk(
  ...
  async (payload: any, { rejectWithValue }) => {
    try {
      const result = await axios.put(`regions/${payload.id}`, {
        id: payload.id,
        data: payload.data,
      });
      return result.data;
...
export const regionsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(uploadCsv.fulfilled, (state) => {
      state.loading = false;
      fulfilledNotify(state, 'Regions has been uploaded');
    });
...
export const { setRefetch } = regionsSlice.actions;
